...
- **디자인 개편(v0.4)** — 동봉 귀여운 폰트(주아·고운돋움, `fonts/`) 기본화, 그라데이션+글로우 파형(`viz_color` 팔레트 UI), `line` 미니멀 비주얼라이저, 이미지 없을 때 흐르는 그라데이션 배경(`bg_style`), 소절 단위 자막 페이드, 썸네일 리디자인. AI 에이전트도 `viz_color`/`bg_style` 변경 가능.
- **연출 확장(v0.5)** — 💿 레코드 모드(`disc`, 첫 배경 이미지를 원형 회전 앨범아트로), 다음 소절 미리보기(`sub_preview`, UI 기본 on), 하단 스크림 자동(배경 이미지 시 가독성), 곡 진행바(`progress_bar`), **AI 자동 팔레트**(`POST /api/palette` {lyrics,title} → viz_color/bg_grad/mood, UI 🎨 버튼). 에이전트 도구에도 전부 노출.
...
- **자동 재시도/예상시간** — 실패 시 1회 재시도(`auto_retry`) + 곡 길이·해상도 기반 예상 렌더시간.
- **히스토리 검색·필터 + 프리셋 IO** — 최근작업 검색/상태필터, 프리셋 JSON 내보내기/가져오기.
- **오디오 반응 배경** — `--bg-pulse`(RMS 엔벨로프→sendcmd→eq 밝기 펄스).
- **큐 대시보드** — 진행중/대기/완료/실패 집계.
- **인트로/아웃트로 병합** — `render.concat_clips`(해상도 정합 + 무음 폴백), 업로드 클립을 본편 앞뒤 concat.
- **✨ 반짝이는 음표 파티클** — `--sparkle`: ♪ 6개가 화면을 은은하게 떠다니며 반짝임(장식용).
- **📢 아웃트로 구독 유도 카드** — `--outro-cta`/`--outro-cta-text`: 곡 끝 ~4초에 문구 페이드인.
- **💿 레코드 배경 스타일** — `--disc-bg-style glow|radial` (레코드 모드 전용):
  glow=앨범아트를 블러+채도업해 배경 전체를 색감으로 채움(밝기는 안 건드림 —
  scrim이 자막 가독성을 이미 담당), radial=디스크 뒤 컬러 헤일로가 음악 RMS에
  반응해 반짝임(조용한 구간↓/큰 구간↑, `--bg-pulse`와 동일한 엔벨로프 재사용).
