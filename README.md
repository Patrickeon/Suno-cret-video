...
- **v0.6 — AI 뮤직비디오** : 🎬 스토리보드 모드(가사→장면별 AI 클립→크로스페이드 배경, `/api/storyboard`), fal.ai provider(공개 API, Kling 등), Mock provider(키 없이 데모), 비주얼라이저 리디자인(슈퍼샘플링·이중 글로우·EQ 막대). Kaiber 는 공개 API 부재 확인(스텁 유지).
