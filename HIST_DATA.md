...
## 현재 상태 (2026-09-22 기준, 같은 날 2라운드 추가)

- **이번 세션 작업 전부 완료+검증됨, 커밋/푸시 완료.** 아래 "### 6.", "### 7." 항목 참고.
- 마지막 실제 커밋 전 HEAD(2라운드 시작 시점): `13b29df feat(disc-theme): lp_vinyl/text_ring 샘플 이미지 매칭 + 스타일 프리셋`
...
- **미완료(다음 세션 확인 필요)**:
  - `square_spin` 테마엔 그림자 미적용(의도적 스킵, 회전각이 임의라 그림자도 같이 돌려야 함).
  - **쇼츠(세로형) 모드에서 lp_vinyl 조합이 프레임보다 살짝 넓음**(양쪽 ~22px 오버플로,
    `disc_diameter()`의 세로형 공식과 `LP_VINYL_SIZE_SCALE` 조합 때문). 대칭이라 눈에 띄게
    깨지진 않지만 완전히 고치려면 `disc_diameter()`(모든 디스크 테마 공유) 를 건드려야 해서
    이번 스코프에선 보류함.
...
### 7. lp_vinyl 레이아웃/입체감 2차 보정 + LP 라벨 별도 이미지 + 이미지 지우기 (2026-09-22, 같은 날 2라운드) — 완료+검증됨, 커밋됨

"### 6."을 커밋한 직후, 사용자가 실제 본인 이미지(애니메이션풍 일러스트)로 렌더한 스크린샷
(`sample/playscreen.png`)을 보여주며 재지적. media-engineer 2회 추가 사용.

- **캡션 "top" 위치 버그(재현·수정)**: `title_caption_pos=top` + `progress_bar_pos=top`을
  같이 쓰면 캡션이 진행바/앨범 커버 위쪽 가장자리와 겹치는 버그를 실제 렌더로 재현
  (1280x720에서 4~65px 겹침 확인) → `title_caption_geometry()`가 진행바 트랙 바닥 기준으로
  캡션 y를 밀어내고, `render()`가 그만큼 `cy`(디스크 중심)를 재계산해서 "진행바 → 캡션 →
  (간격) → 앨범/LP(중앙)"이 겹치지 않는 하나의 레이아웃으로 동작하도록 수정.
  `pos=auto`/진행바 없음 조합은 회귀 없음(렌더로 재확인).
- **lp_vinyl 커버+비닐 조합을 화면 중앙에 정렬**: 기존엔 커버만 중앙 정렬하고 비닐은
  오른쪽으로만 삐져나와 전체 구도가 우측으로 치우쳐 보임 → 커버 중심을 살짝 왼쪽으로
  당겨(bounding box 기준 재계산) 커버+비닐을 합친 전체가 화면 중앙에 오도록 수정.
- **크기 재조정**: `LP_VINYL_SCALE` 0.90→0.96(비닐이 커버와 거의 같은 크기, "커버가 살짝만
  더 크게"), `LP_VINYL_SIZE_SCALE` 1.5→1.3(전체적으로 조금 작게) — 실사 일러스트 이미지로
  직접 렌더해보고 눈으로 비교해서 결정(기존 샘플2 원피팅 실측치보다 사용자 육안 피드백 우선).
- **비닐에 입체감**: `vinyl_shading_expr()` 신설 — 대각선 방향 하이라이트 밴드 + 바깥 테두리
  안쪽 그림자 + 라벨 가장자리 그림자를 그루브/라벨 양쪽에 동일한 광원 방향으로 적용.
- **진행바-앨범 간격 / 제목-아티스트 간격**: 위 캡션 레이아웃 수정과 함께 진행바-앨범 사이
  간격 확보, `art_gap` 배수 1.3→1.6로 제목/아티스트 줄간격 추가 확대.
- **LP 레코드 라벨 이미지 분리(`--disc-lp-art`)**: 앨범 커버와 비닐 라벨에 서로 다른
  이미지를 쓸 수 있게 신규 CLI 플래그 추가, 미지정 시 기존처럼 커버 이미지로 폴백.
  배선: `make_mv.py`(argparse+main()) → `backend/render.py`(`--disc-lp-art` 패스스루) →
  `backend/main.py`(`disc_lp_art` 업로드 필드+검증+저장+opts) → `frontend/app/page.tsx`
  (discTheme==="lp_vinyl"일 때만 보이는 별도 Dropzone).
- **이미지 선택 지우기(✕) 버튼**: `frontend/app/components/ui.tsx`의 공용 `Dropzone`
  컴포넌트에 지우기 버튼 추가 — 모든 업로드 필드(배경/앨범/LP라벨/로고/인트로·아웃트로
  클립/앨범모드)에 자동 적용됨(각 페이지의 `onFiles` 콜백이 이미 빈 배열을 올바르게
  처리하고 있어서 컴포넌트 한 곳만 고치면 전체 반영됨).
- **배경 기본 정책 통일**: "LP 바이닐" 프리셋이 쓰던 플랫 핑크 배경(`discBgStyle:"off"` +
  `bgColor`)을 제거하고 `glow`로 통일 — 배경 이미지를 따로 안 고르면 항상 앨범아트 자체를
  블러 배경으로 쓰는 게(`### 6.`에서 만든 백엔드 폴백) 사용자가 원하는 기본 동작이라는 게
  이번에 명확해짐. text_ring 프리셋과 동일한 정책.
- **검증**: `pytest backend/tests` 166→**180개 전부 통과**. 실제 렌더(`--disc-art`/
  `--disc-lp-art`를 서로 다른 이미지로 지정)로 커버/라벨에 각각 다른 이미지 확인,
  top/auto 캡션 위치 둘 다 프레임 추출로 겹침 없음 확인, `tsc --noEmit`/`npm run lint` 통과.
- **다음 세션 참고**: 위 "현재 상태"의 쇼츠 모드 lp_vinyl 오버플로 미해결 항목 참고.
